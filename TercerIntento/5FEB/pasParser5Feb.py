# -*- coding: utf-8 -*-
"""
Created on Tue Feb  5 15:23:05 2019
...
class PasParser(Parser):
    ...
    def identifier_list(self, p):
        return [ReadLocation(p.IDENTIFIER)] #Location

    @_('identifier_list "," IDENTIFIER')
    def identifier_list(self, p):
        p.identifier_list.append(ReadLocation(p[2])) #[Location]
    ...
    def assignement_statement(self, p):
        ...
        return VarAssign(WriteLocation(p[0]), p[2]) #Statement
    ...
    @_('variable')
    def factor(self, p):
        return SimpleLocationFactor(p[0]) #Expression
